...
interface DateCellPickerProps {
	...
	/** Merged into the trigger, for a caller that wants a button rather than a bare cell. */
	className?: string;
	/** Off for a trigger that brings an icon of its own, so it does not end up wearing two. */
	showIcon?: boolean;
	...
	return (
		<>
			<button
				...
				aria-expanded={ open }
				{ ...getReferenceProps() }
				className={ cn('flex w-full cursor-pointer items-center gap-1 rounded-sm text-left text-sm outline-none', className) }
			>
				{ children ?? <span className="truncate">{ formatLongDate(value) }</span> }
