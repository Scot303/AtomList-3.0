...
export const Button = (props: ButtonProps) => {
	const { className, variant = 'primary', size = 'md', isLoading, leftIcon, children, disabled, ...rest } = props;

	const isGhost = variant === 'ghost' || variant === 'ghost_primary';

	const baseStyles = cn(
		"relative inline-flex items-center justify-center font-bold transition-all active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none overflow-hidden",
		!isGhost && "border"
	);
	...
	const variants = {
		...
		warning: "border-orange-500/50 text-orange-500 bg-orange-500/5 hover:bg-orange-500/10 hover:text-os-orange-500 shadow-orange-500/10",
	...
	return (
		<motion.button
			whileTap={ { scale: 0.98 } }
			className={ cn(baseStyles, variants[variant], isGhost ? ghostSizes[size] : sizes[size], className) }
