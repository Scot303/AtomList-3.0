...
public class TransactionService {
	...
	public TransactionView create(UUID listId, CreateTransactionRequest request) {
		...
		auditLogger.recordOnCommit(securityService.getCurrentUserId(), transaction.getId(), AuditEventType.TRANSACTION_MANAGEMENT, AuditOutcome.SUCCESS, String.format("%s transaction of %s added to list %s.", transaction.getType(), transaction.getTotal(), PaymentListService.describe(list)));
	...
	public TransactionView update(UUID id, UpdateTransactionRequest request) {
		...
		auditLogger.recordOnCommit(securityService.getCurrentUserId(), transaction.getId(), AuditEventType.TRANSACTION_MANAGEMENT, AuditOutcome.SUCCESS, String.format("%s transaction on list %s updated.", transaction.getType(), PaymentListService.describe(transaction.getList())));
	...
	public List<TransactionView> seedInstructorExpenses(UUID listId, SeedInstructorExpensesRequest request) {
		...
		auditLogger.recordOnCommit(securityService.getCurrentUserId(), list.getId(), AuditEventType.TRANSACTION_MANAGEMENT, AuditOutcome.SUCCESS, String.format("%d instructor expense row(s) added to list %s.", created, PaymentListService.describe(list)));
