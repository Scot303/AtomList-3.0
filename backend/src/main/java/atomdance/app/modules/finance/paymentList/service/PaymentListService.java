...
import atomdance.app.modules.finance.paymentList.dto.UpdateCustomListRequest;
...
import atomdance.app.modules.finance.transaction.service.InstructorExpenseService;
...
public class PaymentListService {
	...
	public int recalculateOpenStandardLists() {
		...
		if (!open.isEmpty()) {
			log.info("Rebuilt {} open standard list(s): {}", open.size(), open.stream().map(PaymentListService::describeList).toList());
		}
	 ...
	 */
	public static void requireSameAccount(PaymentList list, Group group) {
	 ...
	 */
	public static String describeList(PaymentList list) {
